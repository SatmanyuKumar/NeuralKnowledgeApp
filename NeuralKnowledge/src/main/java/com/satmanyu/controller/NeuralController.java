...
import com.satmanyu.mail.MailSending;
import com.satmanyu.repo.DocumentRepo;
import com.satmanyu.service.AiService;
import com.satmanyu.service.DocumentService;

import com.satmanyu.service.UserService;
...
public class NeuralController {
	...
	public String myDocument(Model m, Principal principal) {
		...
		List<Document> docs = docService.myDocument(userName);
	...
	@PostMapping("/ask")
	public String ask(@RequestParam Long documentId, @RequestParam String question, Principal principal, Model model) throws IOException, TikaException {

		String username = principal.getName();
		Tika t= new Tika();
		Document file=docService.getDocument(documentId);
		
		

	String content=t.parseToString(new ByteArrayInputStream(file.getDocument()));

//			String answer = aiService.ask(question,documentId);
	String answer = 	aiService.askFromDocument(question, content);
	...
	@PostMapping("/upload")
	public String uploadPdf(@RequestPart MultipartFile file, Principal principle, Model m) throws Exception {
	...
	public String forget(@RequestParam String email, Model m) {
		...
		if (user == null) {
			m.addAttribute("msg", "User Does Not Exist!");
			 ...
			 s = mailSending.mailSendResetPasswordHTML(email);
	...
	public void viewPdf(@RequestParam Long id, HttpServletResponse response,Principal p) throws IOException {
		...
		Document doc = docService.getDocument(id,user).orElse(null);
		if(doc!=null) {
			byte[] docs = doc.getDocument();
